...
object Day04 : AoCApp() {
    ...
    private fun part2(input: List<Pair<List<Int>, List<Int>>>): String {
        return input.count { isListIntersect(it.first, it.second) }.toString()
    }

    private fun processInput(inputLines: List<String>): List<Pair<List<Int>, List<Int>>> {
        return inputLines.map { line ->
            "(\\d+)-(\\d+),(\\d+)-(\\d+)".toRegex().matchEntire(line)!!.groupValues.drop(1).map { it.toInt() }
                .let { Pair((it[0]..it[1]).toList(), (it[2]..it[3]).toList()) }
        }
    }
    ...
    private fun isListIntersect(first: List<Int>, second: List<Int>): Boolean {
